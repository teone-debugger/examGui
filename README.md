```
    @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
    @ ██████╗ ██╗   ██╗███╗   ██╗ ██████╗ ███████╗ ██████╗ ███╗   ██╗███████╗ @
    @ ██╔══██╗██║   ██║████╗  ██║██╔════╝ ██╔════╝██╔═══██╗████╗  ██║██╔════╝ @
    @ ██║  ██║██║   ██║██╔██╗ ██║██║  ███╗█████╗  ██║   ██║██╔██╗ ██║███████╗ @
    @ ██║  ██║██║   ██║██║╚██╗██║██║   ██║██╔══╝  ██║   ██║██║╚██╗██║╚════██║ @
    @ ██████╔╝╚██████╔╝██║ ╚████║╚██████╔╝███████╗╚██████╔╝██║ ╚████║███████║ @
    @ ╚═════╝  ╚═════╝ ╚═╝  ╚═══╝ ╚═════╝ ╚══════╝ ╚═════╝ ╚═╝  ╚═══╝╚══════╝ @
    @                          █████╗ ███╗   ██╗██████╗                       @
    @                         ██╔══██╗████╗  ██║██╔══██╗                      @
    @                         ███████║██╔██╗ ██║██║  ██║                      @
    @                         ██╔══██║██║╚██╗██║██║  ██║                      @
    @                         ██║  ██║██║ ╚████║██████╔╝                      @
    @                         ╚═╝  ╚═╝╚═╝  ╚═══╝╚═════╝                       @
    @         ██████╗ ██████╗  █████╗  ██████╗  ██████╗ ███╗   ██╗███████╗    @
    @         ██╔══██╗██╔══██╗██╔══██╗██╔════╝ ██╔═══██╗████╗  ██║██╔════╝    @
    @         ██║  ██║██████╔╝███████║██║  ███╗██║   ██║██╔██╗ ██║███████╗    @
    @         ██║  ██║██╔══██╗██╔══██║██║   ██║██║   ██║██║╚██╗██║╚════██║    @
    @         ██████╔╝██║  ██║██║  ██║╚██████╔╝╚██████╔╝██║ ╚████║███████║    @
    @         ╚═════╝ ╚═╝  ╚═╝╚═╝  ╚═╝ ╚═════╝  ╚═════╝ ╚═╝  ╚═══╝╚══════╝    @
    @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
```
---
# Progetto Elementi di Ingegneria del Software 2024
## ___Dungeons and Dragons an Old School Adventure rpg___

---
### This game is brought to you by:
- Gianluca Frassoni (GitHub: GianSette)
- Matteo Milan (GitHub: Teone-Debugger)
- Cezar Cosmin Stefan Tordai (GitHub: Mugurel750)
---
### What is Dungeons and Dragons?
___Dungeons and Dragons___ is **not** only a text adventure. <br>
You can play our adventure RPG from Terminal or you can play with the Graphic option. <br>
**You** can choose from three different classes and three different races, pick up potions and much more from the dungeon, and even weapons but you have to make sure if your class can use that weapon ;) <br>
Pay attention to the various enemies in the dungeon, and don't understimate the power of the Coconuts. <br>

___Are you going to defeat the mighty Aurelion Sol?___

---
### How to Download and Install
***Clone the repository or download the .zip from GitHub***

***Windows***
- Download and extract the .zip file and extract the "examGui-main" folder on your desktop <br>
- Then open the cmd and compile using Maven by writing "mvn clean compile" and "mvn exec:java" <br>
- ___Make sure you're in the right folder before compiling___

***MacOs***
- Download and extract the .zip file and extract the "examGui-main" folder on your desktop <br>
- Then open "Finder" and go to "Applications" > "Utilities" and double-click "Terminal" <br>
- ___Make sure you're in the right folder before compiling___

---
### How to Play

 __[You can read the pdf file attached here](MANUALE.pdf)__ <br>
 ***freely inspired by GTA:SAN ANDREAS "Local Business Advertiser's Guide"***

 **We recommend to play with the GUI**

---
### System Documentation
__[Unit Test report](Report.pdf)__ <br>
__[Jira Web](https://studenti-team-zw2x4sa2.atlassian.net/jira/software/projects/DD10/boards/3/backlog?selectedIssue=DD10-2)__ <br>
__[Jira PDF](jira.pdf)__ <br>
__[System Test Document](System_test.pdf)__ <br>
__[System Test Report](System_Test_Report.pdf)__ <br>
__[Design Document](Design.pdf)__ <br>

---
### Rights and Licenses
***Art***
- All the artworks are generated with DALL-E (ChatGpt 4)
- The Ascii art in the top of this file was made using [Ascii Art Archive](https://www.asciiart.eu/image-to-ascii) <br>
- All fonts used are for Personal use Only

___Other components of the project are all made by us___

All Rights Reserved ®
    
